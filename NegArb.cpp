...
bool ErrorHandler(string message, string prevasset, string round, string iter)
{
    ...
    {
        error = true;
        ofstream log("Neg_error_log.txt", ios::app);
    ...
    {
        ofstream log(LogFname, ios::app);
        log << "swap on pending for 25 repeated try. Cannot continue.....exiting"
            << "\n";
        log.close();
    ...
    else if (message[9] == '1' && message[10] == '1' && message[11] == '0' && message[12] == '2') // caused after not enoguh balance response
    {
        ofstream log(LogFname, ios::app);
        error = true;
        log << "Mandatory parameter 'quoteAsset|quoteQty|baseAsset' was not sent, was empty/null, or malformed."
            << "\n";
        log.close();
        /*unlocking transaction*/
        ofstream end("locked.transaction");
...
string SignedResp[5];
string *APIcall(string protocol, string path, unordered_map<string, string> par, string key[], int size, int round, int iter, bool NoProcessing = false, string prevasset = "null")
{
    CURL *curl;
    string queryString;
...
bool order(string from, string to, string prevasset)
{
    ...
    if (stod(FromBalance) > 0)
    {
        ...
        res = APIcall("POST", "/sapi/v1/bswap/swap", par, key, 1, 1, 1, false, prevasset);
            ...
            if (order(a, b, "null"))
            {
                ...
                if (order(b, c, a))
                {
                    ...
                    if (order(c, a, b))
                    {
                        ...
                        string time = asctime(tt);
                        /*---------------t*/
                        profit << 0.07 + net << " " << searched << " " << SpaceRemover(time) << "neg "
                               << "\n";
                        profit.close();
                        ofstream loger(LogFname, ios::app);
                        loger << "-----profitable----------"
...
int main(int argc, char **argv)
{
    ...
    while (controller)
    {
        if (CheckFree())
        {
            ...
            string iterC = argv[5];
            LogFname = "./run_log_neg/neg_" + roundC + " " + iterC + "_" + time + ".txt";
            controller = false;
